...
describe("Method talkAssignedToTrack() returns true and false when expected", () => {
  ...
  test("talkAssignedToTrack() works with arrays", () => {
    ...
    let thisTalkWillReturnFalse = new Talk(
      20,
      "I will return false",
      false,
      4,
    ...
    let thisArrayWillFailTest = [
      loveCoding,
      thisProject,
      dogsAreAwesome,
      thisTalkWillReturnFalse,
    ];

    expect(
      awesomeArrayThatWillPass.every((talk) => talk.talkAssignedToTrack())
    ).toBe(true);

    expect(
      thisArrayWillFailTest.every((talk) => {
...
describe("Method placeTalk() pushes Talks to the correct Track array", () => {
  test("placeTalk() pushes to a session's morning and afternoon correctly", () => {
    ...
    expect(trackExampleOne.sessions.afternoon.talks).toEqual([
      iGoTrackTwo,
      letsGo,
    ]);
  });
